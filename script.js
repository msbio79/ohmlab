    ...
    // 3) 전구 그리기
      ...
      if (state.options.showSymbols) {
        ...
        if (state.options.showValues) {
          ...
          // 위쪽: 저항값 (Ω)
          ctx.fillStyle = isGlowing ? '#ffeb3b' : '#ffffff';
          ctx.fillText(`${this.value.toFixed(0)}Ω`, rx, ry - 35);
...
// CSS 변수를 JS에서 직접 추출하여 캔버스 색상 유지 (브라우저 스타일 타이밍 고려 폴백 추가)
function varColor(cssVarName) {
  const val = getComputedStyle(document.documentElement).getPropertyValue(cssVarName).trim();
  if (val) return val;
  // 폴백 설정
  if (cssVarName === '--accent-cyan') return '#00e5ff';
  if (cssVarName === '--accent-yellow') return '#ffeb3b';
  if (cssVarName === '--accent-pink') return '#ff4081';
  if (cssVarName === '--bg-card-light') return '#20284d';
  if (cssVarName === '--border-color') return 'rgba(255, 255, 255, 0.08)';
  if (cssVarName === '--text-muted') return '#8c9bb4';
  return '#ffffff';
}
...
function solveCircuit() {
  ...
  // 모든 노드에 아주 아주 미세한 전도도(leakage, 1e-9)를 그라운드와의 사이에 설정
  ...
  comps.forEach(c => {
    ...
    if (c.type === 'resistor' || c.type === 'bulb' || c.type === 'ammeter' || c.type === 'voltmeter') {
      ...
      if (c.isOpen) {
        // 스위치가 열렸을 때: 아주 작은 전도도를 설정하여 전류가 흐르지 않게 함
        addConductance(idxA, idxB, 1e-9);
      } else {
  ...
  state.wires.forEach(w => {
    ...
    if (state.options.showElectrons && isFlowing) {
      ...
      if (isSelected) {
        ...
        ctx.strokeStyle = state.options.flowDirection === 'electrons' ? '#ffffff' : varColor('--accent-yellow');
      }
...
function initEventBindings() {
  ...
  // 더블 클릭/더블 탭으로 스위치 조절 및 소자 선택 해제 지원
  canvas.addEventListener('click', (e) => {
    ...
    // 캔버스 빈 곳 클릭 시 선택 해제 또는 도선 선택
    if (!comp && state.interaction.mode === 'none') {
      const clickedTerminal = findTerminalAt(world.x, world.y);
      if (!clickedTerminal) {
        // 단자도 클릭되지 않았음 -> 도선이 있는지 확인
